...
void I2C_WriteByte(unsigned char addr, unsigned char data)
{
    while (LL_I2C_IsActiveFlag_BUSY(OLED_I2C))
        ;                                    // Determine if the bus is busy 判断总线是否忙碌
    LL_I2C_GenerateStartCondition(OLED_I2C); // START 1
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_MODE_SELECT))
        ;                                                                   // Read SR1->SB bit to determine whether the start bit is sent 读SR1->SB位，判断起始位是否发送
    I2C_Send7bitAddress(OLED_I2C, OLED_ADDRESS, I2C_Direction_Transmitter); // Send from device address 发送从设备地址
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
        ;                                 // Waiting for EV6 to complete; Read SR1, SR2 等待EV6完成；读SR1、SR2
    LL_I2C_TransmitData8(OLED_I2C, addr); // Register address of the slave device 从设备的寄存器地址
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
        ;
    LL_I2C_TransmitData8(OLED_I2C, data); // send data 发送数据
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
        ;

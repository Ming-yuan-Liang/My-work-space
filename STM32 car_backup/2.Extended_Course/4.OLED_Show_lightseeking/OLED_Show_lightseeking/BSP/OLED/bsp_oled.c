...
// 向oled写数据 Write data to oled
void I2C_WriteByte(unsigned char addr, unsigned char data)
{
    while (LL_I2C_IsActiveFlag_BUSY(OLED_I2C))
        ;                                    // 判断总线是否忙碌 Determine if the bus is busy
    LL_I2C_GenerateStartCondition(OLED_I2C); // START 1
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_MODE_SELECT))
        ;                                                                   // 读SR1->SB位，判断起始位是否发送 Read the SR1->SB bit to determine whether the start bit is sent
    I2C_Send7bitAddress(OLED_I2C, OLED_ADDRESS, I2C_Direction_Transmitter); // 发送从设备地址 Send slave device address
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED))
        ;                                 // 等待EV6完成；读SR1、SR2 Wait for EV6 to complete; read SR1, SR2
    LL_I2C_TransmitData8(OLED_I2C, addr); // 从设备的寄存器地址 Register address of the slave device
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
        ;
    LL_I2C_TransmitData8(OLED_I2C, data); // 发送数据 send data
    while (!I2C_CheckEvent(OLED_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED))
        ;
